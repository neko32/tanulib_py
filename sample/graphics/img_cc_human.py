...
def main():
    ...
    cc = CascadeClassifier(
        classifier_name=CascadeClassifierFilter.HUMAN_FRONT_FACE,
        min_posible_obj_size=[30, 30],
        max_possible_obj_size=[300, 300]
    )

    tmp_home_dir = os.environ["HOME_TMP_DIR"]
    f_input_name = str(Path(__file__).parent.parent.joinpath("img", "biz_persons.jpg"))
    f_output_name = str(Path(tmp_home_dir).joinpath("cc_human_sample.jpg"))
    ...
    try:
        ...
        print("Cascade Classifier with human front face done.")
