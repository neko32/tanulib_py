...
def main():
    ...
    f_output_name = str(Path(tmp_home_dir).joinpath("boxmean_filter_sample.jpg"))

    if exists(f_output_name):
        remove(f_output_name)

    img = imread_wrapper(f_input_name, cv2.IMREAD_GRAYSCALE)
    boxm_filted = filter_by_box_mean(
        img=img,
        kernel_size=7,
        normalize=True
    )
    try:
        cv2.imwrite(f_output_name, boxm_filted)
        print(f"Box-Mean Filter sample done. file written to {f_output_name}")
